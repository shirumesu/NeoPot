...
export async function recognize(base64, language, options = {}) {
  ...
  const text = await multi_lang_ocr(base64, appid, secret)

  return text.trim()
}

async function multi_lang_ocr(img_base64, appid, secret) {
  const res = await query(img_base64, 'MultiLanguageOCR', '2022-08-31', appid, secret)
  if (res.ok) {
    const result = res.data
    if (result['data']) {
      const data = result['data']
      let texts = ''
      for (const text of data['ocr_infos']) {
        texts += text['text'] + '\n'
      }
      return texts
    }
