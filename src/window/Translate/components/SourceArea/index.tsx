...
export default function SourceArea(props) {
    ...
    const detect_language = useCallback(async (text) => {
        setDetectLanguage(await detect(text));
    }, [setDetectLanguage]);

    const handleNewText = useCallback(async (text) => {
        text = text.trim();
        if (hideWindow) {
            appWindow.hide();
        } else {
    ...
    }, [deleteNewline, incrementalTranslate, recognizeLanguage, recognizeServiceList, hideWindow, handleNewText]);
