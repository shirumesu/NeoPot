...
export default function Translate() {
  ...
  const [rememberWindowSize] = useConfig(
    "translate_remember_window_size",
    false,
  );
  const [translateServiceInstanceList, setTranslateServiceInstanceList] =
    useConfig("translate_service_list", [
      "deepl",
      "bing",
      "lingva",
      "yandex",
  ...
  const [recognizeServiceInstanceList] = useConfig("recognize_service_list", [
    "local_model",
  ]);
  const [ttsServiceInstanceList] = useConfig("tts_service_list", [
    "lingva_tts",
  ]);
  const [collectionServiceInstanceList] = useConfig(
  ...
  const validTranslateServiceInstanceList = Array.isArray(
    translateServiceInstanceList,
  )
    ? translateServiceInstanceList.filter(isValidServiceInstanceKey)
    : [];
  const validRecognizeServiceInstanceList = Array.isArray(
  ...
  const validTtsServiceInstanceList = Array.isArray(ttsServiceInstanceList)
    ...
    : [];
  const validCollectionServiceInstanceList = Array.isArray(
        ...
        if (
          ...
          for (const plugin of plugins) {
            const infoStr = await readTextFile(
              `plugins/${serviceType}/${plugin.name}/info.json`,
              {
                baseDir: BaseDirectory.AppConfig,
              },
            );
  ...
  const loadServiceInstanceConfigMap = async () => {
    try {
      ...
      for (const serviceInstanceKey of validTranslateServiceInstanceList) {
        config[serviceInstanceKey] =
          (await getStoreValue(serviceInstanceKey)) ?? {};
      }
          ...
          {hasInitError ? (
            <div className="rounded-medium border border-danger/30 bg-danger/10 p-4 text-sm text-danger">
              <div className="mb-2 font-semibold">
                Translate window initialization failed
              </div>
            ...
            <>
              ...
              {isServiceConfigReady
                ? validTranslateServiceInstanceList.map(
