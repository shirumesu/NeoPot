...
function serviceError(code: ServiceErrorCode, message: string, provider?: string): ServiceErrorResult {
    return {
        ok: false,
        code,
        provider,
...
export async function translate(request: TranslateRequest): Promise<string | ServiceErrorResult> {
    if (!request.text || !request.from || !request.to) {
        return serviceError('SERVICE_CONFIG_MISSING', 'Translation request is missing text or language fields.', request.provider);
    }
    ...
    switch (request.provider ?? 'google') {
        ...
        default:
            return serviceError('SERVICE_NOT_MIGRATED', 'Provider is not migrated to Electron Main yet.', request.provider);
    }
}
...
export async function recognize(): Promise<ServiceErrorResult> {
    try {
        ...
        return serviceError('SERVICE_NOT_MIGRATED', 'OCR provider is not migrated to Electron Main yet.', 'recognize');
    }

    return serviceError('SERVICE_NOT_MIGRATED', 'OCR provider is not migrated to Electron Main yet.', 'recognize');
}

export async function textToSpeech(text: string, lang: string, config?: { requestPath?: string }): Promise<string | ServiceErrorResult> {
    if (!text || !lang) {
        return serviceError('SERVICE_CONFIG_MISSING', 'TTS request is missing text or language.', 'lingva');
    }

    return ttsLingva(text, lang, config);
}

export async function addToCollection(source: string, target: unknown, config?: { port?: number }): Promise<void | ServiceErrorResult> {
    if (!source) {
        return serviceError('SERVICE_CONFIG_MISSING', 'Collection request is missing source text.', 'anki');
    }

    await addToAnki(source, target, config);
}

export function onServiceStream(eventId: string, callback: (token: StreamToken) => void): () => void {
